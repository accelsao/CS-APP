...
int addOK(int x, int y) {
	...
	int signy=y>>31;
	int sum_sign=(x+y)>>31;
	//only case 0,1 return false
	return (signx^signy)|(!(sum_sign^signx));
}
/* 
...
int bang(int x) {
	int neg_x=(~x)+1;
	// only 0&0==0 otherwise not 0
	// and neg_x and x on sign-bit must be differnet
	int bit_sign=(~(neg_x&(~x)))>>31;
...
int bitMask(int highbit, int lowbit) {
	return ((1<<(highbit+1))+(~0))^((1<<lowbit)+(~0));
}
/* 
...
int byteSwap(int x, int n, int m) {
    int nbyte=n<<3;
    int mbyte=m<<3;
    int no_change = ~ ((0xFF<<nbyte) | (0xFF<<mbyte)); //沒變的兩個 
    return (x&no_change) | (((x>>nbyte)&0xFF)<<mbyte) | (((x>>mbyte)&0xFF)<<nbyte);
}
/* 
...
int copyLSB(int x) {
	return ~(x&1)+1;
	//return (x<<31)>>31;  if sign-extension
}
/*
